...
export async function POST(req: Request,
    ...
    try {
        ...
        if(purchase) {
            return new NextResponse("Already purcahse", { status: 400 });
        }

        if(!course) {
            return new NextResponse("Not found", { status: 404 });
        }
        // define the line items for the stripe checkout page
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            line_items,
            mode: 'payment',
            success_url: `${process.env.NEXT_PUBLIC_APP_URL}/courses/${courseId}`,
